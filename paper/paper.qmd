...
Data on word counts and unique word counts for the four selected authors were sourced from the original text files of the selected authors' writings. The original files of these writings [@citePowell, @citeNan, @citeDH, @citeHu] were gathered through online library sources, these files were processed and transformed into workable data using R[@citeR]. For key operations, please refer to the [@sec-appendix].
